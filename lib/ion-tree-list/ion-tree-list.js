...
function addDepthToTree(obj, depth, collapsed, collapsedLevel) {
    for (var key in obj) {
        if (obj[key] && typeof(obj[key]) == 'object') {
            obj[key].depth = depth;
            obj[key].collapsed = collapsed;
            addDepthToTree(obj[key], key === 'tree' ? ++ depth : depth, collapsedLevel == depth, collapsedLevel)
        }
    }
    return obj
}
    ...
    return {
        ...
        controller: ['$scope', function($scope) {
            ...
            $scope.$watch('items', function() {
                $scope.items = addDepthToTree($scope.items, 1, $scope.collapsed, $scope.collapsedLevel);
